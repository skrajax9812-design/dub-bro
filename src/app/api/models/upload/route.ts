...
import { MODEL_PRESETS, safeModelPath } from "@/lib/models";
...
export async function POST(req: NextRequest) {
  ...
  const fd = fs.openSync(abs, offset === 0 ? "w" : "r+");
  try {
    // A resumed transfer may open a file that does not exist yet.
    if (offset > 0 && fs.fstatSync(fd).size < offset) fs.writeSync(fd, Buffer.alloc(0), 0, 0, offset - 1);
  ...
  return NextResponse.json({ ok: true, path: relPath, bytes: size });
}
